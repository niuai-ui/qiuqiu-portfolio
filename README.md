...
一键发布会依次检查日志目录与 Excel 是否一致、13 列结构、数据行格式和字段是否合法、封面是否存在且为精确 3:4、网站能否构建、JavaScript 语法是否正确。全部通过后才会提交和推送，并等待 GitHub Pages 部署完成。

只有状态为“已发布”的作品会出现在网站。“模组本体链接”填写模组自己的官方发布页；百度网盘链接可以随时在 Excel 中更新。尚无地址时保留空白，不得编造。“模组更新日期”记录原版模组更新，“汉化更新日期”只在汉化文件变化时更新，二者彼此独立。新增记录必须复制标准数据行格式并保持行高 36，预检会拦截格式不一致的行。
